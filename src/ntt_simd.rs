...
//!
//! Activated by the `simd` feature flag. When enabled, [`crate::poly::Poly`]
//! routes its forward/inverse NTT through [`ntt_simd`] / [`invntt_simd`],
//! which are cross-validated against the scalar reference by the unit tests
//! and end-to-end by the KAT suite (`cargo test --features simd`).
//!
//! All SIMD results are congruent mod q to the scalar path; since every
//! caller Barrett-reduces before serialization, congruence is sufficient for
//! bit-identical KEM outputs.
...
/// to process multiple butterflies at once. Falls back to scalar on
...
pub fn ntt_simd(r: &mut [i16; N]) {
    ...
    {
        // SAFETY: NEON is guaranteed on all aarch64 targets Rust supports.
        unsafe { ntt_neon(r) };
    }
    #[cfg(not(any(target_arch = "x86_64", target_arch = "aarch64")))]
    {
...
unsafe fn ntt_avx2(r: &mut [i16; N]) {
    ...
    while len >= 2 {
        ...
        while start < 256 {
            ...
            k += 1;
            if len >= 16 {
                // SAFETY: avx2 available (caller contract); indices in range.
                unsafe { avx2::butterfly_avx2(r, start, len, zeta) };
            } else {
...
unsafe fn invntt_avx2(r: &mut [i16; N]) {
    ...
    use core::arch::x86_64::{__m256i, _mm256_loadu_si256, _mm256_set1_epi16, _mm256_storeu_si256};
    ...
    // SAFETY: avx2 available (caller contract); 16-lane loads stay in bounds.
    unsafe {
        let f_vec = _mm256_set1_epi16(f);
        let mut j = 0;
        while j + 16 <= N {
            let a = _mm256_loadu_si256(r.as_ptr().add(j) as *const __m256i);
            let result = avx2::fqmul_avx2(a, f_vec);
            _mm256_storeu_si256(r.as_mut_ptr().add(j) as *mut __m256i, result);
            j += 16;
        }
    }
}
...
mod neon {
    ...
    /// # Safety
    /// Requires the `neon` target feature (guaranteed on aarch64).
    #[inline(always)]
    pub(super) unsafe fn fqmul_neon(a: int16x8_t, b: int16x8_t) -> int16x8_t {
        unsafe {
            ...
            // result = (a*b - t*Q) >> 16  using widening multiply for mulhi.
            // NEON has no direct mulhi, so widen then narrow-shift by 16.
    ...
    /// # Safety
    /// Requires the `neon` target feature; `start + 2*len <= 256`.
    #[inline(always)]
...
unsafe fn ntt_neon(r: &mut [i16; N]) {
    ...
    while len >= 2 {
        ...
        while start < 256 {
            ...
            k += 1;
            if len >= 8 {
                // SAFETY: NEON available on aarch64; indices in range.
                unsafe { neon::butterfly_neon(r, start, len, zeta) };
            } else {
                for j in start..start + len {
                    let t = fqmul(zeta, r[j + len]);
                    r[j + len] = r[j] - t;
...
unsafe fn invntt_neon(r: &mut [i16; N]) {
    use crate::ntt::ZETAS;
    use crate::reduce::{barrett_reduce, fqmul};
    use core::arch::aarch64::{vdupq_n_s16, vld1q_s16, vst1q_s16};
    ...
    // SAFETY: NEON available on aarch64; 8-lane loads stay in bounds.
    unsafe {
        let f_vec = vdupq_n_s16(f);
        let mut j = 0;
        while j + 8 <= N {
            let a = vld1q_s16(r.as_ptr().add(j));
            let result = neon::fqmul_neon(a, f_vec);
            vst1q_s16(r.as_mut_ptr().add(j), result);
            j += 8;
        }
    }
}
